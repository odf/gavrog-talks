...
\begin{frame}
  \begin{center}
    ...
    For an abstract graph $G$, we have to explicitly pick a translation group
    $T\leq \mathrm{Aut}(G)$.  If $G$ is not crystallographic, $T$ is not
    unique and we can have $\varphi T\varphi^{-1} \neq T$.

    If we can show that $\alpha_\varphi$ can be uniquely defined on some vertex
    sublattice with respect to $T$, everything is good.

    How can we exclude rogue automorphisms that mess up all the sublattices,
    no matter how large?
  \end{center}
\end{frame}
